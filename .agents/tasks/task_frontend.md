...
## Шаг 8. Оценки участников администратором (Админ-оверрайд)

Реализовать интерфейс в панели "Скоринг" для проставления оценок от лица участников команды администраторами.

### 1. Доработка логики и рендеринга деталей эпика (`scoring-panel.js`)
* В методе `renderEpicDetails` проверять роль текущего пользователя (`admin` или `superadmin`).
* Если роль позволяет администрировать:
  * Отображать таблицу **"Оценки участников (Администрирование)"**.
  * Заполнять её списком участников команды из расширенного ответа `GET /api/gantt/epics/{epic_id}/scores` (поле `members`).
  * Для каждого участника выводить ФИО, Роль, Текущую оценку.
  * Для непроголосовавших участников показывать выпадающий список (или ряд кнопок) с оценками SP Фибоначчи и кнопку "Сохранить за участника".
  * Для проголосовавших — текущую оценку и возможность переопределить её.
* В блоке оценки рисков:
  * Если пользователь является админом, отображать список оценок по каждому риску для всех участников команды.
  * Добавить выпадающие списки выбора Вероятности (1-4) и Влияния (1-4) с возможностью отправить оценку от лица любого участника.

### 2. Взаимодействие с API
* Кнопки оценки за участников должны отправлять запросы:
  * Для эпиков: `POST /api/gantt/admin/scores/epic` с телом `{ "epic_id": "...", "user_id": "...", "score": score }`.
  * Для рисков: `POST /api/gantt/admin/scores/risk` с телом `{ "risk_id": "...", "user_id": "...", "probability": prob, "impact": imp }`.
* После успешной отправки вызывать `selectEpic(epic)` для перезагрузки и обновления прогресс-бара и финальных результатов.

---

## Шаг 9. Ввод оценок с клавиатуры целым числом 0-500 (человеко-дни)

Заменить выбор оценок Фибоначчи в веб-интерфейсе на ввод числового значения (человеко-дни, чд) с клавиатуры:
1. В `web/gantt/js/scoring-panel.js` в методе `renderEpicDetails`:
   * Для обычного пользователя заменить сетку кнопок Фибоначчи на блок с числовым инпутом: `<input type="number" id="input-epic-score" class="input" min="0" max="500" placeholder="0-500 чд" style="width: 120px; margin-right: 10px;">` и кнопкой «Сохранить оценку».
   * Для администратора в таблице участников заменить сетку кнопок Фибоначчи в режиме редактирования на аналогичный числовой инпут: `<input type="number" class="input admin-score-input" data-user-id="${m.id}" min="0" max="500" placeholder="чд" value="${selectedVal !== null ? selectedVal : ''}" style="width: 80px; text-align: center; margin-right: 8px;">` и кнопку «Сохранить».
2. Обновить обработчики событий (сохранения оценки):
   * Считывать числовое значение из инпутов, приводить к целому числу, валидировать диапазон `[0, 500]` и отправлять на бэкенд.
3. Локализовать интерфейс:
   * Во всех надписях, таблицах оценок по ролям, баджах итоговой оценки заменить `"SP"` на `"чд"`.

---

## Шаг 10. Квартальное планирование, вместимость, квоты, роли PO/BA и вкладка Отчетов

Внедрить интерфейс для управления квартальным планированием эпиков и отображения отчетов по вместимости команды и квотам на типы задач.

### 1. Форма создания эпика в админ-панели (`admin-panel.js`)
* Расширить форму создания эпика `#form-create-epic`:
  * Добавить числовое поле «Год» (по умолчанию `2026`).
  * Добавить выпадающий список «Квартал» (1, 2, 3, 4, по умолчанию `3`).
  * Добавить выбор «Тип задачи» (Новая функциональность `feature`, Архитектурная задача `architecture`, Техдолг `techdebt`).
  * Добавить чекбоксы «Роли-оценщики» (загружать доступные роли динамически через `apiGet('/roles')`).
* При отправке формы `POST /api/gantt/epics` передавать: `year` (int), `quarter` (int), `type` (string), `evaluating_role_ids` (массив UUID выбранных ролей).

### 2. Новая вкладка «📊 Отчеты»
* В `index.html` добавить вкладку «📊 Отчеты» в навигацию (`nav-tabs`), видимую для всех участников.
* В области содержимого вкладки `#tab-content-reports`:
  * Добавить форму выбора параметров: Команда, Год, Квартал.
  * При изменении параметров делать запрос `GET /api/gantt/reports/capacity?team_id=UUID&year=YYYY&quarter=Q`.
  * Отображать **Таблицу вместимости ролей и эпиков**:
    * Столбцы: Эпик, Тип, Столбцы для каждой роли (показывающие среднюю оценку по роли `weighted_avg`), Итого.
    * Итоговая строка:
      * Доступная трудоемкость (Capacity) по каждой роли и суммарно.
      * Запланировано (Planned) — сумма оценок эпиков по ролям и суммарно.
      * Разница (Недопланировано / Перепланировано). Если разница отрицательная (перепланирование), подсвечивать ячейку ярко-красным цветом.
  * Отображать **Таблицу квот по типам задач**:
    * Столбцы: Тип задач, Лимит %, Фактически %, Статус.
    * Новая функциональность: лимит $\le 40\%$.
    * Архитектурные задачи + Техдолг: лимит $\le 60\%$.
    * Если лимит превышен, выводить красный статус `Превышено`, если соблюден — зеленый `В рамках квот`.

### 3. Реализация логики в `reports-panel.js`
* Создать новый файл [reports-panel.js](file:///Users/krassor/GolandProjects/EpicScoreBot/web/gantt/js/reports-panel.js) и реализовать там всю разметку, обработчики выбора и рендеринг таблиц отчетов.
* Зарегистрировать и проинициализировать модуль в [app.js](file:///Users/krassor/GolandProjects/EpicScoreBot/web/gantt/js/app.js).
...
5. **Багфикс: Зависание на новых эпиках (null reading length)**:
   - В функции `renderEpicDetails` в [scoring-panel.js](file:///Users/krassor/GolandProjects/EpicScoreBot/web/gantt/js/scoring-panel.js) добавить защитную инициализацию массивов по умолчанию (`roleScores = roleScores || []` и `risks = risks || []`), чтобы предотвратить падение при получении `null` значений для эпиков без проставленных оценок/рисков.
