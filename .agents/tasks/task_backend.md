...
## Шаг 8. Квартальное планирование, вместимость, квоты, роли PO/BA и Telegram Bot

Реализовать поддержку квартального планирования, расчета вместимости, контроля квот, конфигурации ролей-оценщиков и пошагового выбора в Telegram Bot.

### 1. Миграция базы данных и моделей
* Создать файл `internal/migrator/migrations/006_quarterly_planning.sql`:
  * Добавить в таблицу `epics` колонки `year INT NOT NULL DEFAULT 2026`, `quarter INT NOT NULL DEFAULT 3`, `type TEXT NOT NULL DEFAULT 'feature'`.
  * Наложить constraints на `quarter` `[1, 4]` и `type` `('feature', 'architecture', 'techdebt')`.
  * Создать таблицу `epic_evaluation_roles (epic_id, role_id)`.
  * Залить роли `PO` и `BA` в справочник ролей.
  * Для существующих эпиков привязать все текущие роли в качестве оценивающих в `epic_evaluation_roles`.
* Обновить `domain.Epic` и репозиторий:
  * В репозитории поддержать запись и чтение года, квартала, типа и списка оценивающих ролей для эпиков.

### 2. Изменение скоринга (Оценивающие роли)
* В методах `TryCompleteEpicScoring` и `TryCompleteRiskScoring`:
  * Вместо `CountTeamMembers` запрашивать количество членов команды, у которых хотя бы одна из ролей входит в список оценивающих ролей данного эпика.
  * Если число фактически полученных оценок меньше этого значения, считать скоринг незавершенным.

### 3. API Отчетов и вместимости
* Создать хэндлер `GetCapacityReport` (эндпоинт `GET /api/gantt/reports/capacity`):
  * Принимает параметры: `team_id`, `year`, `quarter`.
  * Считает Capacity команды и ролей по формуле: $Capacity = Members \times 8 \times 6 \times 0.838$.
  * Находит все эпики команды за выбранный квартал.
  * Суммирует оценки (значения средних из `epic_role_scores`) по ролям и суммарно.
  * Рассчитывает недопланирование/перепланирования (доступная вместимость минус сумма запланированных оценок).
  * Считает квоты по типам задач (feature $\le$ 40%, architecture + techdebt $\le$ 60% от суммы оценок всех эпиков за квартал).

### 4. Доработка Telegram Bot и генерации PDF-отчетов
* **Пошаговый выбор в Telegram**:
  * В `internal/telegram/handlers_epic.go` (`handleReport`) и `admin_callbacks.go`:
    * Изменить callback-цепочку команды `/report`.
    * Выбор команды $\rightarrow$ инлайн-выбор года (например, `2026`, `2027`) $\rightarrow$ инлайн-выбор квартала (`1`, `2`, `3`, `4`).
    * При клике на финальный квартал генерировать PDF-отчет для выбранного квартала.
* **Обновление PDF-отчета**:
  * Обновить метод `GetReportData` и генератор отчетов: в отчет должны входить только эпики выбранного квартала.
  * Добавить в HTML-шаблон отчета таблицы вместимости по ролям (с выводом недопланирования/перепланирования) и квот по типам задач.
