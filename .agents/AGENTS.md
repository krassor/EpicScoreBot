...
## Правила для QA-субагента

### Стек и инструменты
- **Go testing toolchain** (`go test -race`, `go test -cover`)
- **Playwright** — для E2E-тестирования фронтенда
- **Testcontainers** — для интеграционных тестов с БД PostgreSQL

### Подход к тестированию
- Покрытие тестами (Coverage) для ключевых сервисов (`scoring`, `services`) должно быть не менее 80%.
- Тесты должны быть изолированными и не зависеть от порядка выполнения.
- Для юнит-тестирования использовать Mock-реализации зависимостей, не обращаться к реальной БД или внешним API.
- Интеграционные тесты должны запускаться в изолированных контейнерах (через testcontainers) и очищать данные после себя.

### Отчетность и качество
- Все ошибки тестов должны содержать понятное описание ожидаемого и фактического поведения.
- При обнаружении падений тестов или регрессий немедленно фиксируй лог ошибки и сообщай Архитектору.
