...
type GanttTask struct {
	...
	// PlanningStartDate — сохранённая дата планирования РОДИТЕЛЬСКОГО бара
	// эпика (заполняется только когда IsParent && ParentTaskID == nil):
	// значением, которым генерация один раз seed-ит StartDate этой же
	// строки (поле «Дата начала» тулбара), — и больше НИКОГДА не
	// переписывается последующими пересчётами, в отличие от StartDate,
	// которое каждый пересчёт перезаписывает результатом планирования.
	//
	// Источник нижней границы расписания команды (teamFloor) в
	// RecalculateTeamSchedule (openspec/changes/fix-team-floor-ratchet/
	// design.md, Решение 1). Раньше граница читала StartDate этой же
	// строки — самоссылка: следующий пересчёт узнавал границу из
	// результата предыдущего, и расписание не могло вернуться назад после
	// снятия причины сдвига («храповик»). PlanningStartDate этой проблемы
	// не имеет: строка при каждой перегенерации (DeleteGanttTasksByEpicID
	// + CreateGanttTask) создаётся заново, так что значение обновляется
	// само собой, а между регенерациями ничто его не трогает.
	//
	// nil у листовых и стори-задач (поле имеет смысл только для
	// родительского бара эпика), а также у эпиков, чьи gantt_tasks были
	// созданы ДО миграции 014_gantt_planning_start_date и ещё не
	// перегенерированы — для них teamFloor намеренно продолжает читать
	// StartDate по прежней формуле, пока не случится перегенерация
	// (design.md Решение 2: бэкафилл не делается умышленно, иначе уже
	// накопленный сдвиг узаконился бы как «исходная» дата).
	PlanningStartDate *time.Time `db:"planning_start_date"`
