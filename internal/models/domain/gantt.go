...
type TaskAssignment struct {
	...
	StartOffsetDays int `db:"start_offset_days"`
	// NotBeforeDate — ограничение "Начать не ранее": календарная дата,
	// раньше которой задача не может начаться, nil — не задано. Хранится
	// КАК ДАТА, а не как смещение (в отличие от StartOffsetDays) — это
	// внешнее событие с известной датой, оно не должно ехать при пересчёте
	// дат самой задачи или её стори (openspec/changes/
	// add-task-start-constraints/design.md, Решение 5). Действует только
	// как нижняя граница: если задача и без него стартовала бы позже, дата
	// её к себе не притягивает.
	NotBeforeDate *time.Time `db:"not_before_date"`
	// WaitForStoryID/WaitForRoleID — ссылка на другую ролевую задачу
	// команды: текущая задача не начинается раньше завершения задачи,
	// адресуемой этой парой "стори (или эпик без сторей) + роль" — тем же
	// ключом, которым адресуется сама эта строка (EpicID, RoleID), см.
	// design.md Решение 1. Оба поля заданы одновременно либо оба nil —
	// WaitForRoleID без WaitForStoryID (и наоборот) не бывает, это
	// проверяется на уровне сервиса (gantt.Service.SetTaskStartConstraint).
	//
	// Ссылка на исчезнувшую (роль перестала оцениваться, эпик вне периода)
	// задачу НЕ удаляется и не считается ошибкой — строка сохраняется, но
	// планировщик просто не находит дат для цели и не применяет
	// ограничение (design.md Решение 4), как и с UserID на выбывшем из
	// пула человеке. Физическое удаление самой стори/роли, на которую
	// указывает ссылка, снимает её через ON DELETE SET NULL в БД — см.
	// миграцию 013_task_start_constraints.sql.
	WaitForStoryID *uuid.UUID `db:"wait_for_story_id"`
	WaitForRoleID  *uuid.UUID `db:"wait_for_role_id"`
}
