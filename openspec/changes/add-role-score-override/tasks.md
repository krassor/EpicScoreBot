...
- [ ] 5.2 Ручная проверка сценариев из specs/role-score-override/spec.md на реальных данных: переопределение роли, попытка переопределения на незавершённом скоринге, пересчёт по формуле с учётом переопределённой роли, права team-admin чужой команды.
