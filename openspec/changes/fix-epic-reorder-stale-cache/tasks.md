...
- [ ] 2.1 Проверить вручную (браузер): сохранить новый порядок эпиков через модалку, закрыть, открыть снова без перезагрузки страницы — модалка показывает только что сохранённый порядок, а не старый
- [ ] 2.2 Проверить вручную: выполнить reorder эпиков 2-3 раза подряд без перезагрузки страницы между попытками — каждое следующее открытие корректно отражает предыдущее сохранение, дубликаты `sort_order` не появляются заново
- [ ] 2.3 Регрессия: модалки «Порядок историй» и «Порядок ролей» продолжают работать без изменений (эта задача их не касается)
